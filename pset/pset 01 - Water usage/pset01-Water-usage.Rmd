...
for(s in sources){
  ...
  for(year in years){
    ...
    ## Read the csv file by concatenating folder path rawdata/ with the years and source we are looping at with .csv; Show the beginning of the data  frame
    ...
    ## Transform all columns except GEOID to a long format
    df <- df %>% pivot_longer(cols = -GEOID, names_to = "combined", values_to = "value") 
    
    ## Seperate the combined column into three columns: source, crop, and year by '.'
    df <- df %>% separate(col = combined, into = c("source", "crop", "year"), sep = "\\.", remove = TRUE)
    
    ## end of transforming data ############################
    
    ## Append the DataFrame for current year and source to the DataFrame for all years and sources
    d = rbind(d, df) 
  }
...
```{r}
## Group by crop, source, and year and sum the value across location
dd = d %>% group_by(crop, src, year) %>% summarise(value = sum(value), .groups = 'drop') 

## Group by crop and source and calculate manual mean
dd2 = dd %>% group_by(crop, src) %>% mutate(mean = mean(value))

table2 <- dd2 %>% group_by(crop, src) %>% summarize(
    mean = mean(mean),
    value_2008 = sum(value[year == "2008"]),
    value_2020 = sum(value[year == "2020"]),
    .groups = 'drop') %>%
mutate(change = value_2020 - value_2008, percent_change = (change / value_2008)*100)

## Drop the value 2008 and value 2020
table2 <- table2 %>% select(-value_2008, -value_2020)
...
table2$crop <- as.factor(table2$crop)
table2$src <- as.factor(table2$src)

## I decided to filter out crops started with others as it does not provide actual insights and disturb the pattern of data since the other crops is much larger than individual crops
table2_filtered <- table2 %>%filter(!grepl("^other", crop))

## Visualization 1: Mean Bar plots
ggplot(table2_filtered, aes(x = crop, y = mean, fill = src)) +
...
```
```{r}
## Visualization 2: Bar plot of absolute change and percentage change
ggplot(table2_filtered, aes(x = crop, y = change, fill = src)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "Change in Water Usage by Crop and Source",
       x = "Crop",
